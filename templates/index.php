<main class="content__main">
  ...
  <?php
  if ($get == 'td') {
    $classname1 = 'tasks-switch__item--active';
  }
  if ($get == 'all' || $get == '') {
    $classname = 'tasks-switch__item--active';
  }     // тупой метод, надо js
  if ($get == 'tm') {
    $classname2 = 'tasks-switch__item--active';
  }
  if ($get == 'fin') {
    $classname3 = 'tasks-switch__item--active';
  }
  ?>
  ...
  <table class="tasks">
    ...
    <?php } ?>
    <?php
    foreach ($tasks as $val) : (($val['data_stop'] == date('Y-m-d', $d)) and ($val['status'] == 0)) ? $classname = "task--important" : $classname = "";
      ($val['status'] == 1) ? $classname_st = "task--completed" : $classname_st = ""; ?>
      <tr class="tasks__item task <?= $classname, $classname_st; ?>">
        <!-- task--completed -->
